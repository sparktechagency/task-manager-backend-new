...
const getAllPaymentByPoster = catchAsync(async (req, res, next) => {
  const { userId } = req.user;
  const result = await paymentService.getAllPaymentByPosterService(req.query, userId);
  // // console.log('result',result)
  ...
  if (result) {
    sendResponse(res, {
      statusCode: httpStatus.OK,
      success: true,
      message: 'Poster history are retrived Successfull!!',
      data: result,
    });
  } else {
    sendResponse(res, {
...
const getAllEarningByPaymentMethod = catchAsync(async (req, res) => {
  ...
  const result:any = await paymentService.filterBalanceByPaymentMethod(userId);
  // console.log('result', result);
